...
public class CustomCalcPresenter {
    ...
    void attach(CustomCalcDialog v, Bundle state) {
        view = v;
        ...
        if (settings.decimalSep == CustomCalcDialog.FORMAT_CHAR_DEFAULT) {
            settings.decimalSep = dfs.getDecimalSeparator();
        }
        if (settings.groupSep == CustomCalcDialog.FORMAT_CHAR_DEFAULT) {
        ...
        if (state == null) {
            ...
            if (settings.initialValue != null) {
                resultValue = CustomCalcDialogUtils.stripTrailingZeroes(settings.initialValue
    ...
    void onDigitBtnClicked(int digit) {
        ...
        boolean withinMaxInt = (pointPos == -1 && (settings.maxIntDigits == CustomCalcDialog.MAX_DIGITS_UNLIMITED
                || valueStr.length() < settings.maxIntDigits));
        boolean withinMaxFrac = (pointPos != -1
                && (settings.maxFracDigits == CustomCalcDialog.MAX_DIGITS_UNLIMITED
    ...
    private void calculate() {
        ...
        if (CustomCalcDialogUtils.isValueOutOfBounds(resultValue, settings.maxValue)) {
            setError(ERROR_OUT_OF_BOUNDS);
            return;
        }

        resultValue = CustomCalcDialogUtils.stripTrailingZeroes(
